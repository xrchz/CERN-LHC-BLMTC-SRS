...
The $\mathsf{RS}\;D\;n\;t$ models the running sums and $\mathsf{update\_time}\;n\;t$ checks if it is the time to refresh the contents of slice $n$.
{\bf should we explain why we have both output function and RS?}

The formal definitions of the functions listed in Table~\ref{tab:descriptions} are given in Figure~\ref{fig:definitions}.
Note that the definition of $\mathsf{input}$ when $n=0$ or when $n>6$ does not change the structure presented. %-- so we use definitions convenient for proving theorems.
...
\begin{align*}
\mathsf{tap}\;0\;0&=0&\mathsf{tap}\;0\;1&=0\\
\mathsf{tap}\;1\;0&=1-1&\mathsf{tap}\;1\;1&=2-1\\
\mathsf{tap}\;2\;0&=8-1&\mathsf{tap}\;2\;1&=16-1\\
