...
The function $\mathsf{last\_update}\;n\;t$ specifies the time that the $n$-th slice last updated, and $\mathsf{exact}\;D\;n\;x\;t$ computes the exact sum of consecutive input counts, without delay, that $\mathsf{output}\;D\;n\;x\;t$ is supposed to approximate (so $\mathsf{exact}$ is to $\mathsf{true\_sum}$ as $\mathsf{output}$ is to $\mathsf{RS}$).
...
\begin{proof}
The shift register cells get their values from the previous cell but they only update every $\mathsf{delay}\;n$ time steps. Therefore\footnote{We do not go through the details here of what happens when $t$ is small, for example $t<m\times\mathsf{delay}\;n$.},
\begin{equation*}
...
Since a slice's output stays constant between update times for the slice, Theorem~\ref{output_input_at_update_times} characterises all outputs at all times.
...
\end{theorem}
\begin{proof}
Follows from Theorem~\ref{output_input_at_update_times}, using the definition of $\mathsf{exact}$, and the fact that $\mathsf{output}\;D\;n\;x\;t$ has the same value as at the last update time.\qed
\end{proof}
