...
A sketch of the desired statement is \[\forall{n}.\,\text{\textsf{RS} }n=\text{true sum}\pm\text{acceptable error}\]
<need a formal model of all components of this equation>
<we only care about 0 <= n <= 11 for the current SRS, but by making the statement forall n, we obtain a generic (parameterized) model>
<start with "RS" - need to model the SRS structure>
<we decide to work with discrete time steps and encode the state of the whole SRS structure as a collection of values, one for each component, at each time step>

<two phases: simplified model, more realistic model, plan to do even more realistic model later>
<the overall sketch of the theory for the more realistic model followed that of the simplified model, but it's not completely clear that if we actually saved time/effort by doing it that way>
