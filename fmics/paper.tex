...
The ionization chambers produce an electrical signal, based on the recording of shower particles, which are readout by \emph{acquisition cards}.
Acquisition cards, located also in the tunnel and thus implemented by radiation-tolerant electronics, acquire and digitize the data and transmit the digitized data to the surface above the tunnel using optical links.
At the surface, data processing cards named \emph{BLETCs} receive the data and decide whether or not the beam should be permitted to be injected or continue circulating.
...
Given the tolerance acceptable for quench prevention, the quench threshold versus loss duration is approximated by a minimum number of sliding integration windows (called \emph{Running Sums}) fulfilling the tolerance.
...
\begin{figure}[t]
  \centering \scalebox{0.65}{ \includegraphics{srs-modified.eps}}
   \caption{The block diagram showing the implementation of SRS in BLETC.}
  \label{fig:srs}
\end{figure}
...
As shown in Table~\ref{fig:SRS-table}, cascading 6 slices is enough to reach the 100 second integration limit requested by the specifications given by the scientists and engineers who conceived the machine protection strategy for the LHC.
...
We model each building block of the SRS structure that holds a count value -- for example, each cell in each shift register -- as a function in HOL\footnote{Here, use of acronym HOL refers to the formal notation that can be parsed by HOL4. In this paper, we use a slightly variant of this notation to improve readability.} that takes the input stream and current time step as inputs and returns the value the corresponding building block holds at that time.
...
The function $\mathsf{true\_sum}$ is easily defined in terms of $\mathsf{exact}$, that is $\mathsf{true\_sum}\;D\;n\;t = \mathsf{exact}\;D\;\left(\left\lfloor\frac{n}{2}\right\rfloor+1\right)\;(n\operatorname{mod}2)\;t$.
...
Our HOL4 proof script for verification of SRS consists of 800 lines of code. We proved 69 theorems, including 14 definitions. In addition, we had to prove approximately 30 generic theorems that were added to HOL4 libraries during this work.
