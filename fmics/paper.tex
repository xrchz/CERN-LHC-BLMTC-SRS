...
Our interest in using formal methods was originally motivated by questions about the SRS that arose in the course of an external technical audit of the BLMS performed by two of the co-authors, Ghafari and Joyce, and their colleagues.
...
\emph{(a) Receive, Check, and Compare (RCC)}: The RCC block receives data directly from the acquisition cards, and attempts to detect erroneous transmissions by using Cyclic Redundancy Check and 8B/10B algorithms~\cite{CRC,8B10B}.
...
There are 32 levels (0.45 to 7 TeV) of beam energy and each processing module holds data only for those 16 connected detectors.
...
%<What about ACL2?>
While offering less ``push-button'' automation than other kinds of formal verification such as model-checking, machine-assisted theorem proving using HOL4 is appropriate for verifying the SRS, 
since it generates correctness theorems; in addition, it gives a way to very explicitly parameterize the model.
%since it is unlikely that the correctness theorems, which need lemmas proved by manually guided mathematical induction, could be generated automatically.
...
$\;\mathsf{tap}\;3\;0=32-1\quad$ &  $\mathsf{tap}\;3\;x=128-1$\\
$\;\mathsf{tap}\;4\;0=32-1\quad$ &  $\mathsf{tap}\;4\;x=256-1$\\
$\;\mathsf{tap}\;5\;0=16-1\quad$ &  $\mathsf{tap}\;5\;x=64-1$\\
$\;\mathsf{tap}\;6\;0=32-1\quad$ &  $\mathsf{tap}\;6\;x=128-1\quad\text{(where each $x>0$)}$\\
$\;\dots\quad$ & $\mathsf{tap}\;n\;x=0\quad\text{(where $x\geq0$ and $n>6$)}$\\[1ex]
...
Thus, the SRS structure's outputs are equal to the exact running sums of the input counts over windows whose sizes depend on the width of the slice's shift register and position of the taps.
...
Under such a constraint, we have proved the follwoing result bounding the error of $\mathsf{output}\;D\;n\;x\;t$ compared to $\mathsf{exact}\;D\;n\;x\;t$.
...
|\mathsf{output}\;D\;n\;x\;t-\mathsf{exact}\;D\;n\;x\;t|\leq(\mathsf{tap}\;n\;x+1)\times(\mathsf{delay\;n})\times k\times\\
(\mathsf{delay\_sum}\;n-1+t\operatorname{mod}\mathsf{delay}\;n)
\end{multline*}
Furthermore, for all $k$ there exists an input stream $D_k$ satisfying $|D_k\;(t'+1)-D_k\;t'|\leq k$ for all $t'$ and 
\begin{multline*}
|\mathsf{output}\;D_k\;n\;x\;t-\mathsf{exact}\;D_k\;n\;x\;t|=(\mathsf{tap}\;n\;x+1)\times(\mathsf{delay\;n})\times k\times\\
(\mathsf{delay\_sum}\;n-1+t\operatorname{mod}\mathsf{delay}\;n)
\end{multline*}

for all $n>0$, $x$, and $t>(\mathsf{tap}\;n\;x+1)\times(\mathsf{delay}\;n)+\mathsf{delay}\;n+\mathsf{delay\_sum}\;n$.

\end{theorem}
\begin{proof}
By applying Theorem~\ref{output_eq_exact}, we are left with an inequation involving the function $\mathsf{exact}$ 
%but not $\mathsf{output}$
specifically between $\mathsf{exact}\;D\;n\;x\;t$ and $\mathsf{exact}\;D\;n\;x\;(\mathsf{last\_update}\;n\;t+1-\mathsf{delay\_sum}\;n)$.
...
The function $\mathsf{exact}$ is defined as a sum of consecutive values of $D$; we need to bound the difference between one such sum and another earlier one of the same size.
...
(We use the fact that $\mathsf{last\_update}\;n\;t=t-t\operatorname{mod}\mathsf{delay}\;n$.)
The input stream achieving this bound is given by $D_k\;t=k\times t$. \qed
\end{proof}

Theorem~\ref{max_error} gives a bound on the error of a running sum at a given time in terms of the time step, the slice and tap numbers, and the assumed bound on the difference between consecutive counts.
This bound is tight for the conditions we assume, namely that the time step is sufficiently high and that the difference between consecutive counts is bounded, in the sense that it is achievable by an input stream satisfying those conditions.
To determine whether an error bound is acceptable with respect to the specification of SRS, however, it turns out to be more useful to know the relative size of the error as a fraction of the true sum. 
According to the specification of SRS, the running sums should have a maximum 20\% relative error (($|\mathsf{RS}\;D\;n\;t - \mathsf{true\_sum}\;D\;n\;t| < 20\%\times(\mathsf{true\_sum}))$.
%To determine whether an error bound is safe, however, it turns out to be more useful to know the relative size of the error as a fraction of the true sum.
We have not yet characterized the relationship between our error bound at a given time and the true sum at that time.
...
By contrast, the results in this paper apply to all possible input streams, not just the sample inputs considered at CERN -- our main contribution, as stated in Theorem 2, showed that the behavior of the SRS is correct for all possible input streams.
However, we do not know if the constraint on input stream given in Theorem 3 is sufficient to satisfy the maximum 20\% relative error bound.
...
{\bf Acknowledgment:} The authors thank Mike Gordon at the University of Cambridge for his insightful and helpful feedback during the course of this work. 
