...
To prove Theorem~(\ref{output_input_at_update_times}), we first make the following observations, for sufficiently large $t$:
\begin{enumerate}
\item
$\mathsf{SR}\;D\;n\;(m+1)\;t=\mathsf{SR}\;D\;n\;m\;(t-\mathsf{delay}\;n)$, since shift register cells get their values from the previous cell, but only update every $\mathsf{delay}\;n$ time steps;
\item
$\mathsf{SR}\;D\;n\;m\;t=\mathsf{SR}\;D\;n\;0\;(t-(m\times\mathsf{delay}\;n))$, by induction on $m$ using the result above;
\item
$\mathsf{output}\;D\;n\;x\;t$ is the sum of values of consecutive shift register cells (Theorem~(\ref{output_sum})), by induction on $t$;
\item
Combining the last two results, $\mathsf{output\;D\;n\;x\;t}$ is the sum of consecutive values of the first shift register cell, $\mathsf{SR\;D\;n\;0}$;
\item
<Thus Theorem~(\ref{output_prev_at_update_times}), because the first shift register cell gets its value from the output of a previous slice>
\item
<Now that we have expressed one output in terms of a previous output, Theorem~(\ref{output_input_at_update_times}) can be proved by induction on $n$ (but this is not trivial - need to check what else apart from $\ref{output_prev_at_update_times}$ is really needed for the inductive step)>
\end{enumerate}
