...
|\mathsf{output}\;D\;n\;x\;t-\mathsf{exact}\;D\;n\;x\;t|\leq(\mathsf{tap}\;n\;x+1)\times(\mathsf{delay\;n})\times k\times(\mathsf{delay\_sum}\;n-1+t\operatorname{mod}\mathsf{delay}\;n)
\end{multline*}
Furthermore, for all $k$ there exists an input stream $D_k$ satisfying $|D_k\;(t'+1)-D_k\;t'|\leq k$ for all $t'$ and $|\mathsf{output}\;D_k\;n\;x\;t-\mathsf{exact}\;D_k\;n\;x\;t|=(\mathsf{tap}\;n\;x+1)\times(\mathsf{delay\;n})\times k\times(\mathsf{delay\_sum}\;n-1+t\operatorname{mod}\mathsf{delay}\;n)$ for all $n>0$, $x$, and $t>(\mathsf{tap}\;n\;x+1)\times(\mathsf{delay}\;n)+\mathsf{delay}\;n+\mathsf{delay\_sum}\;n$.
\end{theorem}
\begin{proof}
By applying Theorem~\ref{output_eq_exact}, we are left with an inequation involving the function $\mathsf{exact}$ but not $\mathsf{output}$, specifically between $\mathsf{exact}\;D\;n\;x\;t$ and $\mathsf{exact}\;D\;n\;x\;(\mathsf{last\_update}\;n\;t+1-\mathsf{delay\_sum}\;n)$.
...
Theorem~\ref{max_error} gives a bound on the error of an SRS output at a given time in terms of the time step, the slice and tap numbers, and the assumed bound on the difference between consecutive counts.
