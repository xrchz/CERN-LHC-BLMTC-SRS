...
With these refinements, our statement becomes \[\forall{D\,n\,t}.\,\text{\textsf{RS} }D\,n\,t=\text{sum, given input stream $D$, for output $n$ at time $t$}\pm\text{acceptable error}\]
...
In both cases, however, the model is simultaneously a simplification and a generalization of the actual structure in the following sense: we model an infinite number of shift registers, each with an infinite number of cells, simply by not specifying their widths or any limits.
<how does that work>
<it's not completely clear that if we actually saved time/effort by doing the simplified one first...>
