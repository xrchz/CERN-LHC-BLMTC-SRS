...
Our goal is to build a generic model of the SRS structure, and to prove that it satisfies its specification, i.e. it calculates approximate running sums of received count values within acceptable error margins.
...
Let $\mathsf{output} \, n$ denote the sum of the last $m$ received count values and $\mathsf{RS} \, n$ denote one of the outputs computed by the slices in the structure of SRS which is supposed to compute the sum of the last $m$ received count values with some delay.
A sketch of the desired correctness statement is \[\forall{n} \cdot \, \mathsf{RS} \,n= \mathsf{output} \, n \pm\text{acceptable error}\]
Although Figure~\ref{fig:srs} suggests that in the SRS structure there are only twelve outputs  (i.e. $1\leq{n}\leq11$), we would obtain a more generic result (that is useful in future upgrades of the system) if we prove the correctness of the above statement for all values of $n$.


%To prove the equation above, we need to formalize all of its terms.
In order to make the above correctness statement more precise, we need to include the notion of time.
The count values arrive at the input of the SRS block every 40 micro-seconds that is abstracted by one time step in our logical model.
We formalize the input stream to SRS as a function of time: $D\;t$ denotes the input value to the SRS structure at time $t$.
In addition, the terms in the above statement depend on the stream of input counts over time.
With these refinements, the correctness statement is defines as: \[\forall{D\,n\,t} \cdot \,\mathsf{RS} \,D\,n\,t = \mathsf{output} \,D\,\,n\, t \pm\text{acceptable error}\]
...
Our main approach in building the logical model of SRS (in both phases) was to model each building block of the SRS structure that holds a count value--for example, each cell in each shift register--as a function in HOL that takes the input stream and current time step as inputs and returns the value which the corresponding building block holds at that time.
...
\item Simplifying timing by ignoring some of the delays that each building block may introduce,
...
In both cases, however, the model is simultaneously a simplification and a generalization of the actual structure of SRS in BLMS in the following aspects: we model an infinite number of slices (rather than six slices), each with an infinite number of shift register cells and taps (rather than fixed width shift registers and only two taps), simply by letting indices range over the natural numbers without explicitly giving limits. Such a parameterization would make the model and the analysis applicable to the future upgraded/modified versions of the system.
