...
In order to achieve the required \emph{dynamic range} (domain of variation of losses), the detectors use both Current-to-Frequency converter and Analogue to Digital convertor circuitries.
The data processing block merges these two types of data coming from a detector so as to send a single value, referred to as a \emph{count}, to the SRS block.
...
\begin{enumerate}
\item The shift register of each slice (except the first) is updated once the shift register of its previous\footnote{Here, previous slice refers to the slice whose output is used as input to this slice.} slice is completely updated, that is, when a count value has propagated down the full length of the previous slice's shift register.
