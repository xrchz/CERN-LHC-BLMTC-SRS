...
The work described here could have been done using other systems such as HOL Light [ref], Isabelle/HOL~\cite{Isabelle}, ProofPower [ref], PVS [ref] or Coq [ref].
The first three of these support essentially the same version of higher-order logic as HOL4, whilst PVS and Coq support more powerful logics.
<What about ACL2?>
While offering less ``push-button'' automation than many other kinds of formal verification such as model-checking, machine-assisted theorem proving is the most appropriate approach to verifying the SRS, since it is unlikely that the correctness theorems, which need lemmas proved by manually guided mathematical induction, could be generated automatically. 

The main objective of formalizing the functionality of SRS was to keep the topology parameterized.
It is very probable that in the future the configuration of the slices and shift registers changes to accommodate higher precision.
If the model is parameterized then, the same model can be used in the future uses of the system.
