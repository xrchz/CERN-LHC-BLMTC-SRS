...
\institute{Critical Systems Labs, Vancouver, BC, Canada
  \and University of Cambridge, Cambridge, UK \and CERN, Geneva, Switzerland}
...
This paper describes a collaborative effort in which the HOL4 theorem prover is being used to formally verify properties of a complex structure used in the FGPA implementation of an integral element of the Large Hadron Collider (LHC) machine protection system at the European Organization for Nuclear Research (CERN). This structure, known as the \emph{Successive Running Sums} (SRS), generates the primary input to the decision logic that must initiate a critical action by the LHC machine protection system in response to the detection of a dangerous level of beam particle loss. The use of mechanized logical deduction complements an intensive study of the SRS structure using simulation. We are especially interested in using logical deduction to obtain a generic result that will be applicable to variants of the SRS structure. This collaborative effort has individuals with very diverse backgrounds ranging from theoretical physics to system safety. One interesting result is the extent to which the use of a formal method has compelled the stakeholders to clarify intricate details of the SRS structure and behaviour.  


%The strategy for machine protection of the Large Hadron Collider (LHC) at the European Organization for Nuclear Research (CERN) is mainly based on the Beam Loss Monitoring (BLM) system. At each turn of the beam, several thousands of data is recorded and processed in order to decide whether the beam should be permitted to continue circulating or it should be extracted due to a dangerous beam loss that may result in significant equipment damage.Such a decision is based on the analysis of loss patterns in time and the beam energy.  The real-time analysis of the recorded data is mainly performed by the \emph{Successive Running Sums} (SRS) technique which allows many and long integration periods to be maintained for each detector's data.  

%In this case study, our eventual goal is to use HOL4 theorem prover to show the correctness of the SRS technique in with respect to BLM specification.   

%we use HOL4 theorem prover to show the correctness of the SRS technique in BLM. We present the formulation of a parameterized model of SRS in HOL4 and explain the verification strategy and results. {\bf what did we learn through this exercise from verification perspective?}
...
The Large Hadron Collider (LHC) at the European Organization for Nuclear Research (CERN) provides a head-on collisions of protons at a center of a mass energy of 14 TeV for high energy particle physics research. %The LHC is installed in a 27 km long LEP tunnel. 
In order to reach the required magnetic field strengths, superconducting magnets cooled with superfluid helium are used. In addition to the energy stored in the LHC, the 700 MJ stored in the circulating beams could initiate quenches (transition of the superconductors to normal conducting state) and cause damages if only a very small fraction of the circulating beam particles deposit their energy in the equipment. The consequences of such event ranges from several hours of down time for cooling down the magnets to reach the superconducting state again, to months of repairs in the case of equipment damage. 

The strategy for machine protection of the LHC is mainly based on the Beam Loss Monitoring (BLM) system. At each turn of the beam, BLM records several thousands of data and process them in order to decide to signal the safe extraction of the beam if losses exceed quench level thresholds. The processing involves a proper analysis of the loss pattern in time and the energy of the beam.
